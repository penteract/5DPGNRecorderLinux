...
def get_mem(pid):
    ...
    with open("/proc/" + pid + "/maps", mode="rb") as mapsfile:
        for l in mapsfile.readlines():
            nextm = re.match(
                rb'([0-9A-Fa-f]+)-([0-9A-Fa-f]+) ([^ ]+) ([0-9A-Fa-f]+) ([0-9A-Fa-f]+:[0-9A-Fa-f]+) ([^ ]+) +([^ ]+)',
                l)
...
class GameRecorder(memlayout.DI):
    ...
    def resultString(self):
        ...
        if r == "NotStarted":
            ...
            return "1/2-1/2"
        elif r == "EndedWhiteWon":
            return "1-0"
        elif r == "EndedBlackWon":
            return "0-1"
        else:
            return "error"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        try:
            ...
            print(
                "can't read process memory, try running something like 'python3 recordgame.py ~/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/common/5dchesswithmultiversetimetravel/5dchesswithmultiversetimetravel'",
                file=sys.stderr)
            raise e
